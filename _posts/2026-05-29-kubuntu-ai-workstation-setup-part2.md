---
title: "Kubuntu 26.04 LTS AI Stack: Ollama, Open WebUI & n8n (Part 2)"
date: 2026-05-29
categories:
  - homelab
  - ai
permalink: /homelab/kubuntu-ai-setup-part2/
tags:
  - ollama
  - docker
  - n8n
...
This is Part 2 of the Kubuntu 26.04 LTS AI Workstation setup. Part 1 covered OS installation, BIOS configuration, ROCm, and the GPU override variables. This post covers the full AI stack: Ollama for local LLMs, Open WebUI for a browser-based chat interface, and n8n for agentic AI automation.

[Back to Part 1: OS Installation and ROCm](/home-lab-blog/homelab/kubuntu-ai-setup-part1/)
...
During inference you should see `GPU%` at 90-97% and `VRAM%` increasing as the model loads. If GPU memory stays at 0, the `HSA_OVERRIDE_GFX_VERSION` variable was not applied correctly to the service.
...
> Do not use `newgrp docker` on KDE Plasma with Wayland. It interferes with the compositor's input handling and can freeze your mouse. A reboot applies the group change cleanly and permanently.
...
If missing, repeat the systemd override step in Step 7 and restart the service.
...
**Mouse freezing after group changes**

Do not use `newgrp` on KDE Plasma with Wayland — it freezes mouse input. Always use `sudo reboot` after `usermod -aG` commands.
